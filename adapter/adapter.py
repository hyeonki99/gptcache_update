...
def adapt(llm_handler, cache_data_convert, update_cache_callback, *args, **kwargs):
    ...
    if cache_enable and not cache_skip:
        ...
        if len(cache_answers) != 0:
            ...
            # 수정한 부분
            hit_callback_detail = kwargs.pop("hit_callback_detail", None)
            if hit_callback_detail and callable(hit_callback_detail):
                factor = max_rank - min_rank
                payload = []
                for d in cache_answers:
                    rank = float(d[0])
                    search_data = d[2]  # (distance, data_id)
                    cache_data = d[3]
                    distance = None
                    data_id = None
                    if isinstance(search_data, (tuple, list)) and len(search_data) >= 2:
                        distance = float(search_data[0])
                        data_id = search_data[1]

                    payload.append(
                        {
                            "question": cache_data.question,
                            "rank": rank,
                            "rank_norm": (rank / factor) if factor else rank,
                            "distance": distance,
                            "data_id": data_id,
                        }
                    )
                hit_callback_detail(payload)
