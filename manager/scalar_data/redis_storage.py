...
def get_models(global_key: str, redis_connection: Redis):
    ...
    class EmbeddingType:
        ...
        # 기존의 validate 메서드 -> Pydantic v1 호환
        # @classmethod
        ...
        # 수정된 validate 메서드 -> Pydantic v2 호환
        @classmethod
        def validate(cls, v, _info=None):  
            if isinstance(v, cls):
                return v
            # redis-om/pydantic 조합에서 커스텀 타입이 dict로 감싸져 들어오는 경우가 있습니다.
            # 예: {"data": "..."}
            if isinstance(v, dict):
                if "data" in v:
                    return cls.validate(v["data"], _info=_info)
                if "__root__" in v:
                    return cls.validate(v["__root__"], _info=_info)
                
            if isinstance(v, np.ndarray):
                # bytes -> latin-1 문자열로 저장(현재 to_numpy 구현과 일치)
                return cls(v.astype(np.float32).tobytes().decode("latin-1"))
            
            if isinstance(v, bytes):
                return cls(v.decode("latin-1"))
            
            # embedding이 list[float]로 들어오는 경우도 안전하게 처리
            if isinstance(v, list):
                return cls.validate(np.asarray(v, dtype=np.float32), _info=_info)
    ...
    class Questions(JsonModel, index=True):
        """
    ...
    class Sessions(JsonModel, index=True):
        """
        session collection
        """
    ...
    class Report(JsonModel, index=True):
        """
        Report collection
        """
        ...
        extra: Optional[str] = None # 수정된 부분
